...
pub fn find_codex_home() -> std::io::Result<AbsolutePathBuf> {
    let dopax_home_env = std::env::var("DOPAX_HOME")
        .ok()
        .filter(|val| !val.is_empty());
    let codex_home_env = std::env::var("CODEX_HOME")
        .ok()
        .filter(|val| !val.is_empty());
    find_codex_home_from_env(dopax_home_env.or(codex_home_env).as_deref())
}

fn find_codex_home_from_env(codex_home_env: Option<&str>) -> std::io::Result<AbsolutePathBuf> {
    // Honor the `DOPAX_HOME` or `CODEX_HOME` environment variable when it is set to allow users
    ...
    match codex_home_env {
        Some(val) => {
            ...
            let metadata = std::fs::metadata(&path).map_err(|err| match err.kind() {
                ...
                _ => std::io::Error::new(
                    err.kind(),
                    format!("failed to read DOPAX_HOME/CODEX_HOME {val:?}: {err}"),
                ),
            })?;
            ...
            if !metadata.is_dir() {
                ...
                let canonical = path.canonicalize().map_err(|err| {
                    std::io::Error::new(
                        err.kind(),
                        format!("failed to canonicalize DOPAX_HOME/CODEX_HOME {val:?}: {err}"),
                    )
                })?;
...
mod tests {
    ...
    fn find_codex_home_without_env_uses_default_home_dir() {
        let resolved =
            find_codex_home_from_env(/*codex_home_env*/ None).expect("default DOPAX_HOME");
