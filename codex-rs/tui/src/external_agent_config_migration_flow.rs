...
pub(crate) const EXTERNAL_AGENT_CONFIG_MIGRATION_FINISHED_MESSAGE: &str =
    "Codex import finished. Run /import again to check for additional items.";
pub(crate) const EXTERNAL_AGENT_CONFIG_MIGRATION_NO_ITEMS_MESSAGE: &str =
    "No Codex setup was found to import.";
pub(crate) const EXTERNAL_AGENT_CONFIG_MIGRATION_REMOTE_UNAVAILABLE_MESSAGE: &str = "Import from Codex is unavailable in remote sessions. Start Dopax locally and run /import.";
pub(crate) const EXTERNAL_AGENT_CONFIG_MIGRATION_DAEMON_UNAVAILABLE_MESSAGE: &str = "Import from Codex is unavailable while Dopax is connected to the local app-server daemon. Stop the daemon, restart Dopax, and run /import.";
...
fn external_agent_config_migration_success_message(remaining_item_count: usize) -> String {
    let message = "Codex import started. You can keep working while it finishes. Imported setup will apply to new chats.";
...
pub(crate) async fn handle_external_agent_config_migration_prompt(
        ...
        .await
    {
        ...
        Err(err) => {
            ...
            return Err(format!("Could not check for Codex setup: {err}"));
        }
    };
